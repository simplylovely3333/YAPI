...
k.scene("floor8", () => {
  ...
  addNPC(330, 440, CHARS.dana, () => {
    ...
    if (state.act1CounterPatchDone) {
      openDialog("ДАНА", "Ты нашёл руководство и собрал контр-патч? Отлично. Это не победа, но теперь у нас есть рычаг: NEXAI придётся спрашивать человека там, где он привык молча назначать решение.", [
        { text: "Что дальше?", action: () => openDialog("ДАНА", "Теперь покажу, почему я торопилась. Мой старый локальный агент DANNA.local знает о твоём патче, хотя он offline. Если это правда — NEXAI уже рядом с ним. Если ложь — кто-то хочет, чтобы мы поверили в неправильного врага.", [
          { text: "Показать агент", action: () => openDialog("DANNA.local", "› network: offline\n› manual patch detected\n› suggestion: не доверяй первому объяснению\n› next: protect floor_8 pipeline", [
            { text: "Дана, он видит патч", action: () => {
              state.danaAgentSeen = true;
              state.task = "Задача: вернуться на 7 этаж — нужен диагностический канал";
              syncHUD();
              logLine("DANNA.local увидел контр-патч, хотя сеть отключена.");
              logLine("Дана подтвердила: следующий риск — пайплайн 8-го этажа.");
              openDialog("ДАНА", "Я этого не писала. И он всё ещё offline. Возвращайся на 7-й: нужен диагностический канал, иначе NEXAI откатит наш патч снаружи.", [
                { text: "На 7 этаж", action: clearDialog }
              ]);
            } }
          ]) }
        ]) },
        { text: "Почему это третье задание?", action: () => openDialog("ДАНА", "Потому что первые два дали знания: логи показали слабости, книга объяснила, как их использовать. Третье задание превратило знания в инструмент. Теперь начинается плохая часть.", [
